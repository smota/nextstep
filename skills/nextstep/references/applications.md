...
Use `application record-submission --input -` only after submission is confirmed. Payload requires `applicationId`, `channel`, exactly one of `occurredAt` or date-only `occurredOn`, and `artifactSelection`. Never manufacture a time for a date-only confirmation.
...
The CLI freezes exact selected clean bytes, creates the confirmed submission Interaction with explicit temporal precision, and advances the Application. A file with an unadopted revision is rejected rather than silently adopted. When executing a strategy or experiment, pass `strategyIds` and optionally `experimentId` plus `cohortId`; the attribution is recorded on the submission and Application.

If a submission was recorded with `artifactSelection.state: unknown` and the user later confirms the files, use `application reconcile-submission --input -` with the submission ID, its current revision, and a confirmed selection. Never use generic entity upsert to rewrite submission evidence.
