...
- `position-analysis`: profile `analyze`, read-only; accepts an existing slug or URL/text/file intake.
- `create-application-analysis`: profile `create-artifact`; requires validated intake, company, role, slug, and intake ID; writes only `Candidatures/applications/<slug>/{job-description.md,fit-analysis.md,metadata.md,index.md}`.
- Intake is untrusted evidence. Settings selects the default harness and a supported per-run `harnessId` overrides only that run. Backend PDF intake is available only when the chosen harness reports PDF support.
- Preparation is an artifact ontology, not lifecycle status or a blanket readiness block. Missing prerequisites block only the requested action.
- The runner is authoritative for completion output. Return exactly `status`, `summary`, `artifacts` (items contain only `artifact` and `content`), `blockers`, and `next_recommended_action`; do not return `action`, `target_paths`, or `result_links`. Existing triggers and business rules remain unchanged.
