...
- `interview-prep`: harness-neutral profile `create-artifact`; uses the selected harness or a supported per-run override.
- Requires `Candidatures/applications/<slug>/{job-description.md,fit-analysis.md}` and may write only `interview-prep.md`. Preparation readiness is independent of lifecycle.
- Backend PDF export is available for supported Markdown output. Preserve submitted source versions. The runner is authoritative: return exactly `status`, `summary`, `artifacts` (items contain only `artifact` and `content`), `blockers`, and `next_recommended_action`; do not return `action`, `target_paths`, or `result_links`. Existing triggers and business rules remain unchanged.
