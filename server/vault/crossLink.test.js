...
test('cross links expose safe scoped opportunity objects from physical storage scope',()=>{
 ...
 assert.deepEqual(companies[0].referencedByApplications,[{slug:'active-role',scope:'active',label:'Active role',resource:'/api/applications/active-role?scope=active'},{slug:'old-role',scope:'archive',label:'Old role',resource:'/api/applications/old-role?scope=archive'},{slug:'../escape',scope:'archive',label:'../escape',resource:null}])
