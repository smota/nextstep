...
export function safeApiMessage(value, fallback = 'Request could not be completed') {
...
export function sendPublicError(res, error) {
  ...
  const message = status >= 500 ? 'Request failed safely' : safeApiMessage(error?.message)
