...
export const PATHS = {
  ...
  recordsDir: path.join(CANDIDATURES_DIR, 'records'),
  artifactsDir: path.join(CANDIDATURES_DIR, 'artifacts'),
  indexesDir: path.join(CANDIDATURES_DIR, 'indexes'),
  reportsDir: path.join(CANDIDATURES_DIR, 'reports'),
  candidaturesConfigDir: path.join(CANDIDATURES_DIR, 'config'),
...
export function validateConfiguredPaths(paths = PATHS, env = process.env) {
  ...
  const vaultRoot = physical(paths.vaultRoot)
  const stateRoot = physical(paths.stateRoot)
  const appRelative = path.relative(appRoot, vaultRoot)
  ...
  const stateRelative = path.relative(appRoot, stateRoot)
  ...
  const dataRelative = path.relative(vaultRoot, stateRoot)
