...
function link(slug,saved){const q=new URLSearchParams({tab:'documents',artifact:saved.artifact});if(saved.version)q.set('version',saved.version);return {path:`Candidatures/applications/${slug}/${saved.filename}`,label:saved.filename,url:`/opportunities/${slug}?${q}`,artifact:saved.artifact,version:saved.version||null,revision:saved.revision}}
...
function acquireAll(paths,artifacts,deps,taskId,operation='create-artifact'){return acquireVaultLocks({vaultRoot:paths.vaultRoot,lockDir:paths.locksDir||path.join(paths.vaultRoot,'.coordination','locks'),targets:artifacts,agent:'nextstep-ui',taskId,operation,runTool:deps.lockCommand})}
