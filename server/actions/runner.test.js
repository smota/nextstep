...
test('catalog exposes six harness-neutral action contracts', () => {
  const runner = new ActionRunner({ vaultRoot: process.cwd(), executable: 'pi', spawnFn: fakeSpawn([]) })
  ...
  for (const action of catalog) {
    assert.ok(action.skill.endsWith('/SKILL.md'))
  ...
  assert.equal(catalog.find(a=>a.id==='create-cv').skill, '.agents/skills/resume-tailor/SKILL.md')
})
...
test('validation returns exact structured prerequisite states for every action', async t=>{
  ...
  const runner=new ActionRunner({vaultRoot:root,executable:'pi',spawnFn:fakeSpawn([])})
  ...
  const unavailable=new ActionRunner({vaultRoot:root,runtime:{selected:()=>({available:false,unavailableReason:'not installed'})},spawnFn:fakeSpawn([])}).validate('position-analysis',{intake:{source:{path:'source.txt'}}});assert.equal(unavailable.prerequisites.find(x=>x.id==='execution-harness').state,'unavailable')
...
test('runner stages, applies, erases, verifies and durably replays apply idempotency',async t=>{
 ...
 const restarted=new ActionRunner({vaultRoot:root,paths,executable:'pi',spawnFn});assert.equal(restarted.apply(started.id,{expectedRevision:0,idempotencyKey:'apply-key'}).state,'completed');assert.throws(()=>restarted.apply(started.id,{expectedRevision:1,idempotencyKey:'apply-key'}),e=>e.statusCode===409);assert.throws(()=>restarted.apply(started.id,{expectedRevision:0,idempotencyKey:'other-key'}),e=>e.statusCode===409)
})

test('prose and expectedWrites never become artifact links',async()=>{const output='prose Candidatures/applications/acme/cv.md\n<<<FINAL_JSON>>>{"status":"completed","summary":"ok","artifacts":[],"blockers":[],"next_recommended_action":null}<<<END_FINAL_JSON>>>';const spawnFn=()=>{const child=new EventEmitter();child.stdout=new PassThrough();child.stderr=new PassThrough();child.kill=()=>true;queueMicrotask(()=>{child.stdout.write(output);child.emit('close',0)});return child};const runner=new ActionRunner({vaultRoot:process.cwd(),executable:'pi',spawnFn});const run=runner.start('position-analysis',{intake:{type:'text',source:{path:'missing'}}});await new Promise(r=>setImmediate(r));assert.deepEqual(runner.get(run.id).artifactLinks,[]);assert.equal(Object.hasOwn(runner.get(run.id),'expectedWrites'),false)})

test('skill contracts preserve triggers and platform boundaries', async () => {
  const contracts = [
    ['job-description-analyzer', ['analyze this job','am I qualified','match score','should I apply']],
    ['interview-prep-generator', ['interview prep','prepare for interview','STAR stories','interview questions','behavioral questions']],
    ['application-pipeline-manager', ['new candidature','new application','application workflow','pipeline','status','archive','company profile','people profile']],
    ['resume-tailor', ['tailor resume','customize resume','target role','specific job','match job description']],
  ]
  const root=PROJECT_ROOT
  for(const [skill,triggers] of contracts){const text=await readFile(path.join(root,'.agents','skills',skill,'SKILL.md'),'utf8');for(const trigger of triggers)assert.ok(text.includes(trigger),`${skill} lost trigger: ${trigger}`);for(const key of ['status','summary','artifacts','blockers','next_recommended_action'])assert.ok(text.includes(`\`${key}\``),`${skill} missing runner key ${key}`);assert.match(text,/do not return `action`, `target_paths`, or `result_links`/);const action=skill==='resume-tailor'?'create-cv':skill==='interview-prep-generator'?'interview-prep':'review-pipeline',artifacts=action==='create-cv'?[{artifact:'cv',content:'x'}]:action==='interview-prep'?[{artifact:'interviewPrep',content:'x'}]:[];assert.doesNotThrow(()=>parseFinalPayload(`<<<FINAL_JSON>>>${JSON.stringify({status:'completed',summary:'ok',artifacts,blockers:[],next_recommended_action:null})}<<<END_FINAL_JSON>>>`,action,{existingSlug:true}))}
  const joined=(await Promise.all(contracts.map(([skill])=>readFile(path.join(root,'.agents','skills',skill,'SKILL.md'),'utf8')))).join('\n')
  for(const deprecated of ['`analyze-job`','`prepare-interview`','`review-pipeline`']) assert.equal(joined.includes(deprecated),false)
