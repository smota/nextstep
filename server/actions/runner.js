...
import { FINAL_PAYLOAD_SCHEMA, FINAL_PAYLOAD_DELIMITERS, parseFinalPayload, applyExistingArtifacts, createApplicationArtifacts, hashApplyKey, hashApplyIntent, readApplyReceipt } from './artifacts.js'
...
export const ACTIONS = Object.freeze({
  'position-analysis': {
    id: 'position-analysis', label: 'Analyze a position', skill: '.agents/skills/job-description-analyzer/SKILL.md', mode: 'read_only', mutability: 'read_only', profile: 'analyze', requiresSlug: false, acceptsSource: true, intakeTypes: ['url','text','file'], backendPdfAvailable: true,
  ...
  'generate-fit-analysis': {
    id: 'generate-fit-analysis', label: 'Generate fit analysis', skill: '.agents/skills/job-description-analyzer/SKILL.md', mode: 'write', mutability: 'governed_write', profile: 'create-artifact', requiresSlug: true,
  ...
  'create-cv': {
    id: 'create-cv', label: 'Create tailored CV', skill: '.agents/skills/resume-tailor/SKILL.md', mode: 'write', mutability: 'governed_write', profile: 'create-artifact', requiresSlug: true,
  ...
  'generate-cover-letter': {
    id: 'generate-cover-letter', label: 'Generate cover letter', skill: '.agents/skills/cover-letter-generator/SKILL.md', mode: 'write', mutability: 'governed_write', profile: 'create-artifact', requiresSlug: true,
  ...
  'interview-prep': {
    id: 'interview-prep', label: 'Create interview preparation', skill: '.agents/skills/interview-prep-generator/SKILL.md', mode: 'write', mutability: 'governed_write', profile: 'create-artifact', requiresSlug: true,
  ...
  'propose-profile-change': {
    id: 'propose-profile-change', label: 'Propose profile change', skill: '.agents/skills/holoself/SKILL.md', mode: 'write', mutability: 'proposal_only', profile: 'create-artifact', requiresSlug: false, requiredInputs: ['claim'],
...
export class ActionRunner extends EventEmitter {
  constructor({ vaultRoot, runtime = null, executable, argsPrefix = [], timeoutMs = 120000, cancellationTimeoutMs = 2000, outputLimit = 512*1024, spawnFn = spawn, killSpawnFn = spawn, platform = process.platform, paths = null, invalidate = () => {}, rollbackInvalidate = () => {}, store = null, intakeResolver = null }) {
    super(); this.vaultRoot = vaultRoot; this.runtime = runtime; this.executable = executable; this.argsPrefix = argsPrefix; this.timeoutMs = timeoutMs; this.cancellationTimeoutMs=cancellationTimeoutMs; this.outputLimit = outputLimit; this.spawnFn = spawnFn; this.killSpawnFn = killSpawnFn; this.platform = platform; this.paths=paths; this.invalidate=invalidate; this.rollbackInvalidate=rollbackInvalidate; this.intakeResolver=intakeResolver
  ...
  catalog() { const selected=this.runtime?.selected(); const available=this.runtime?selected?.available:Boolean(this.executable); return Object.values(ACTIONS).filter(a=>a.id!=='propose-profile-change').map(({ prompt, ...action }) => ({ ...action, networkIsolation:'unverified', selectedHarness:selected?.id||'pi', available, unavailableReason:available?null:(selected?.unavailableReason||'Selected harness is unavailable; action execution is disabled.') })) }
  validate(id, input = {}) {
  ...
  start(id, input = {}) {
    ...
    const root=path.resolve(this.vaultRoot), candidates=[path.resolve(root,action.skill),path.resolve(root,'..',action.skill),path.resolve(PROJECT_ROOT,action.skill)]
