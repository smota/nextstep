...
export const COMMAND_CONTRACTS = Object.freeze({
  ...
  'artifact contract-check': read({ 'data-root': { type: 'absolute-path' }, artifact: { type: 'typed-id', required: true }, template: { type: 'typed-id', required: true } }),
  ...
  'application submission-plan': read({ 'data-root': { type: 'absolute-path' }, id: { type: 'typed-id', required: true } }, ['Advisory only; the caller must explicitly state whether transmitted artifacts are unknown, confirmed absent, or confirmed by ID.']),
  'application record-submission': mutation(['applicationId', 'channel', 'artifactSelection'], { applicationId: { type: 'typed-id' }, channel: { type: 'string' }, occurredAt: { type: 'date-time' }, occurredOn: { type: 'date' }, artifactSelection: { type: 'object', required: ['state'], properties: { state: { enum: ['unknown', 'confirmed_none', 'confirmed'] }, artifactIds: { type: 'array', items: { type: 'typed-id' } } } }, interactionId: { type: 'typed-id' }, note: { type: 'string' }, strategyIds: { type: 'array' }, experimentId: { type: 'typed-id' }, cohortId: { type: 'string' } }, ['Supply exactly one of occurredAt or occurredOn. Artifact IDs are required only for a confirmed selection.']),
  'application reconcile-submission': mutation(['submissionId', 'artifactSelection'], { submissionId: { type: 'typed-id' }, artifactSelection: { type: 'object', required: ['state'], properties: { state: { enum: ['confirmed_none', 'confirmed'] }, artifactIds: { type: 'array', items: { type: 'typed-id' } } } } }, ['Envelope expectedRevision is required. Only an unknown artifact selection can be reconciled.']),
